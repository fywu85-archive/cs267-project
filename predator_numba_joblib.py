...
def outer_loop_batch(rank, y, arg):
    ...
    for idx in range(end - begin):
        ...
        _vzx += __vzx
        _vzy += __vzy
    return _dydt, _vzx, _vzy, begin, end

def f(t, y, arg):
    joblib_segments = Parallel(n_jobs=NPROC)(
        delayed(outer_loop_batch)(rank, y, arg) for rank in range(NPROC)
    )
    ...
    vzy = 0
    dydt = np.zeros((2*(N+1),))
    for segment in joblib_segments:
        _dydt, _vzx, _vzy, begin, end = segment
        vzx += _vzx
        vzy += _vzy
